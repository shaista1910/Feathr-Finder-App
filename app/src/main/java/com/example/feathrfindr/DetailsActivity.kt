...
import com.example.feathrfindr.Utils.Utils
...
class DetailsActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (intent.hasExtra("id")) {
            ...
            uploadImageView.setImageBitmap(intent.getStringExtra("id")
                ?.let { Utils.getBitmapFromMemCache(it) })
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == 1 && resultCode == RESULT_OK && data != null && selectedImage != null) {
            try {
                ...
                intent.getStringExtra("id")?.let { Utils.addBitmapToMemoryCache(it, bitMap) }
            } catch (e: Exception) {
                e.printStackTrace()
            }
        }
